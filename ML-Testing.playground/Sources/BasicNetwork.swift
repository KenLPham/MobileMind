...
open class BasicNetwork: NSObject {
    ...
    public func train () {
        ...
        while totalError > minError {
            ...
            let pEw3: Float = etn1 * dsdh2 * inputs[0]
            let newW3: Float = self.weights[0][2] - n*pEw3 // 0.24978134 (slightly off)
            
            let pEw4: Float = etn1 * dsdh2 * inputs[1]
            let newW4: Float = self.weights[0][3] - n*pEw4 // slightly off
    ...
    private func calculateOutputLayer (output o: (a1: Float, a2: Float)) -> (Float, Float) {
    ...
    private func error (output o: (Float, Float)) -> Float {
